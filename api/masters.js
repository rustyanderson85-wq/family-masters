...
async function fetchSportsData() {
  ...
  const players = (data.Players || []).map(p => {
    const rounds = (p.Rounds || []).map((rd, i) => {
      ...
      (rd.Holes || []).forEach(h => {
        if (h.Score != null) {
          holes[`h${h.Number}`] = h.Score;
          vspar += (h.ToPar || 0);
        }
      });
      const hasHoles = Object.keys(holes).length > 0;
      return { round: rd.Number || (i+1), holes, score: rd.Score || 0, vspar: hasHoles ? vspar : (rd.ToPar || 0) };
    });
    const totalVsPar = rounds.reduce((sum, r) => sum + r.vspar, 0);
    return {
